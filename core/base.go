...
import (
	...
	"git.ma-al.com/goc_marek/pocketbase/tools/logger"
...
func (app *BaseApp) initAuxDB() error {
	...
	// (see https://git.ma-al.com/goc_marek/pocketbase/issues/5607)
	...
	app.OnModelAfterDeleteSuccess().Bind(&hook.Handler[*ModelEvent]{
		...
		Func: func(e *ModelEvent) error {
			if m, ok := e.Model.(FilesManager); ok && m.BaseFilesPath() != "" && supportFiles(e.Model) {
				...
				// (https://git.ma-al.com/goc_marek/pocketbase/discussions/5246#discussioncomment-10128955)
